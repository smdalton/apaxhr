...
def get_mock_ros_form(employee,expired=False, has_image=False):
...
class RosFormTestCase(TestCase):
    ...
    def test_ros_form_is_not_expired(self):
        valid_ros_form = get_mock_ros_form(create_mock_user(), has_image=False)
        self.assertEqual(valid_ros_form.expired, False)

    def test_ros_form_is_expired(self):
        expired_ros_form = get_mock_ros_form(create_mock_user(), expired=True)
        self.assertEqual(expired_ros_form.expired, True)

    def test_ros_form_data_complete(self):
        complete_ros_form = get_mock_ros_form(employee=create_mock_user(), has_image=True)
        self.assertEqual(complete_ros_form.data_complete, True)

    def test_ros_form_data_not_complete(self):
        incomplete_ros_form = get_mock_ros_form(create_mock_user(), has_image=False)
